...
static void wav_timer(int data);
static void tape_wav_state(void)
{
	switch (state) {
	...
	case 2: // on
		...
		if (tape_motor&&tape_play) {
			state=3;
			tape_timer=timer_pulse(1.0/tape_sample->smpfreq, 0, wav_timer);
			break;
		}
...
static void wav_open(char *name)
{
	FILE *fp;
...
static void wav_write(int data)
{
...
//	tape_wav_state(); // removing timer in timer puls itself hangs
}

static void prg_timer(int data);
static void tape_prg_state(void)
{
	switch (state) {
	case 0: break; // not inited
	case 1: // off
	...
	case 2: // on
		...
		if (tape_motor&&tape_play) {
			state=3;
			tape_timer=timer_set(0.0, 0, prg_timer);
			break;
		}
		if (tape_motor&&tape_record) {
			state=4;
...
static void prg_open(char *name)
{
	FILE *fp;
	int i;
...
static void prg_write(int data)
{
#if 0
...
/* 01 prg id
	...
	xor chksum */
static void tape_prgheader(void)
{
	static int i=0;
	...
	case 5:
		...
		break;
	case 6:
		if (i!=192-5-1) { tape_byte();i++;break; }
...
static void zip_timer(int data);
static void tape_zip_state(void)
{
	switch (state) {
	case 0: break; // not inited
	case 1: // off
	...
	case 2: // on
		...
		if (tape_motor&&tape_play) {
			state=3;
			tape_timer=timer_set(0.0, 0, zip_timer);
			break;
		}
		if (tape_motor&&tape_record) {
			state=4;
...
static void zip_open(char *name)
{
...
static void tape_state(void)
{
	switch (tape_type) {
	case TAPE_WAV: tape_wav_state();break;
	case TAPE_PRG: tape_prg_state();break;
	case TAPE_ZIP: tape_zip_state();break;
	}
}
...
{
	if (state==4) {
		if (tape_type==TAPE_WAV) wav_write(data);
		if (tape_type==TAPE_PRG) prg_write(data);
		if (tape_type==TAPE_ZIP) prg_write(data);
	}
}
...
void vc20_tape_motor(int data)
{
	tape_motor=!data;
	tape_state();
//	DBG_LOG(1,"tape",(errorlog,"motor write %d\n",data));
}
...
void vc20_tape_status(char *text, int size)
{
	text[0]=0;
	switch (state) {
	case 4: strcpy(text,"Tape saving");break;
	case 3:
		switch (tape_type) {
		case TAPE_WAV:
			sprintf(text,"Tape (%s) loading %d/%dsec",
				tape_imagename,tape_pos/tape_sample->smpfreq,
					tape_sample->length/tape_sample->smpfreq);
			break;
		case TAPE_PRG:
			sprintf(text,"Tape (%s) loading %d", tape_imagename,tape_pos);
			break;
		case TAPE_ZIP:
			sprintf(text,"Tape (%s) loading %d", tape_imagename,tape_pos);
			break;
